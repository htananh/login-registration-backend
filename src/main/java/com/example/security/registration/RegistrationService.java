...
public class RegistrationService {
	...
	public String register(RegistrationRequest registrationRequest) {
		Boolean isVaidEmail = emailValidator.test(registrationRequest.getEmail());
		if (!isVaidEmail)
		{
		...
		String token= appUserSevice.SignUpUser( new AppUser (
		...
		emailSender.send(registrationRequest.getEmail(), 
				buildEmail(registrationRequest.getLastName(), link));
		return token;
	}
	
	@Transactional
	public String confirm(String token) {
	    ConfirmationToken confirmationToken = confirmationTokenSevice.getToken(token)
	    ...
	    try {
	        ...
	        if (confirmationToken.getExpiresAt().isBefore(LocalDateTime.now())) {
	            throw new IllegalAccessException("Email Expired");
	        }
	        
	        confirmationTokenSevice.setConfirm(token);
	        appUserSevice.enableAppUser(confirmationToken.getAppUser().getEmail());
